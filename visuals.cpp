...
void Visuals::StatusIndicators( ) {
	...
	if (g_aimbot.m_fake_latency) {
		Indicator_t ind{ };
		float ping =  g_menu.main.misc.fake_latency_amt.get();
		Color r =  255;
		Color g = 165;
		Color b =  0;
...
void Visuals::ManualAntiAim()
{
	...
	if (g_hvh.m_left)
	{
		...
		render::rect_filled(x - 53, y - 7 - 2 + 8, 1, 8, color);
		render::rect_filled(x - 54, y - 7 + 2, 1, 5, color);
		render::rect_filled(x - 54, y - 7 + 7, 1, 6, color);
	...
	else if (g_hvh.m_back)
	{
		...
		render::rect_filled(x - 10 + 4, y + 60 - 11, 6, 1, color);
		...
		render::rect_filled(x - 10 + 5 + 5, y + 60 - 10, 6, 1, color);
		render::rect_filled(x - 10 + 5 + 5, y + 60 - 11, 7, 1, color);
	...
	else if (g_hvh.m_forward)
	{
		...
		render::rect_filled(x - 10 + 3, y - 60 + 14, 1, 1, color);
		render::rect_filled(x - 10 + 5 + 5, y - 60 + 10, 6, 1, color);
		render::rect_filled(x - 10 + 5 + 4, y - 60 + 11, 8, 1, color);
		render::rect_filled(x - 10 + 5 + 8, y - 60 + 12, 4, 1, color);
		render::rect_filled(x - 10 + 5 + 10, y - 60 + 13, 3, 1, color);
		render::rect_filled(x - 10 + 5 + 12, y - 60 + 14, 1, 1, color);
	}
