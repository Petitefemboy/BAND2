...
ulong_t __stdcall Client::init(void* arg) {
	...
	// welcome the user.
	g_notify.add(tfm::format(XOR("welcome band$"), g_cl.m_user));
...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("band$ | rtt: %ims | rate: %i | %s"), ms, rate, time.str().data());
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::burgundy, XOR("[band$] "));
...
void Client::Skybox() {
	...
	switch (g_menu.main.misc.skyboxchange.get()) {
	case 0: //Tibet
